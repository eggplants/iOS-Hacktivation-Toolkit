...
while true; do
  ...
  read -r -p "PRESS ENTER ONCE YOU HAVE REGISTERED YOUR SERIAL AND HAVE A PIN LOCKED SIM"
  ...
  while true; do
    ...
    if [ -z "$result" ]; then
      ...
      read -r -p 'INSERT PIN LOCKED SIM AND PRESS ENTER!'
      sleep 5
  ...
  read -r -p "RETURN TO MENU? [ Y / N ] : " check
